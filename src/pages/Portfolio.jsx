...
const TypeIcon = ({ type, symbol }) => {
  const icons = { crypto: CRYPTO_ICONS[symbol] || CRYPTO_ICONS.DEFAULT, etf: 'show_chart', manual: 'savings', stock: 'monitoring', stable: 'attach_money', patrimony: 'home_work' };
  return <span className="material-symbols-outlined text-[20px] text-primary">{icons[type] || 'account_balance'}</span>;
};
...
// ─── Dashboard de portafolio invertible ──────────────────────
...
const Dashboard = ({ v3 }) => {
  ...
  // Targets sincronizados con portfolioAnalysis.js via export
      ...
      <div className="dash-status-bar">
        ...
        <div className="dash-status-item">
          ...
          <div className="dash-alert-pills">
            {critical > 0 && <span className="dash-alert-pill critical">{critical} crédtica{critical > 1 ? 's' : ''}</span>}
                ...
                <div key={role} className="dash-role-row">
                  ...
                  <div className="dash-role-bar-wrap">
                    <div className="dash-role-bar-track">
                      <div className="dash-role-bar-fill" style={{ width: `${Math.min(pct, 100)}%`, background: color, opacity: 0.7 }} />
                    </div>
                  </div>
                ...
                <div key={role} className="dash-target-row">
                  <div className="dash-target-head">
                    ...
                    <div className="dash-target-meta">
                      ...
                      {ok ? <CheckCircle2 size={11} className="text-emerald-400" /> : <XCircle size={11} className="text-rose-400" />}
                    </div>
                  </div>
                  <div className="dash-target-track">
                    <div className="dash-target-fill" style={{ width: `${Math.min(current, 100)}%`, background: ok ? color : '#f43f5e', opacity: 0.75 }} />
      ...
      <div className="dash-two-col">
            ...
            {alertList.map(({ key, label, critical: isCrit }) => {
              ...
              return (
                <div key={key} className={`dash-alert-row ${active ? (isCrit ? 'dash-alert-row--critical' : 'dash-alert-row--warn') : ''}`}>
                  ...
                  {active && <span className={`dash-alert-badge ${isCrit ? 'critical' : 'warn'}`}>{isCrit ? 'CRÍTICO' : 'AVISO'}</span>}
                </div>
              );
            })}
          </div>
        ...
        <Section eyebrow="Métricas cuantitativas" title="Riesgo y retorno">
          <div className="dash-metrics-grid">
            <div className="dash-metric-card"><span className="dash-metric-label">Riesgo medio</span><span className="dash-metric-value">{(risk?.portfolioRisk ?? 0).toFixed(2)}</span></div>
            <div className="dash-metric-card"><span className="dash-metric-label">Retorno est.</span><span className="dash-metric-value green">{(risk?.expectedReturn ?? 0).toFixed(1)}%</span></div>
            <div className="dash-metric-card"><span className="dash-metric-label">HHI</span><span className={`dash-metric-value ${(risk?.hhi ?? 0) > 0.25 ? 'red' : ''}`}>{(risk?.hhi ?? 0).toFixed(4)}</span></div>
            <div className="dash-metric-card"><span className="dash-metric-label">Cash drag</span><span className="dash-metric-value muted">{(risk?.cashDrag ?? 0).toFixed(4)}</span></div>
          </div>
          <div className="dash-class-list" style={{ marginTop: '0.75rem' }}>
            {classRows.map(([cls, pct]) => (
              <div key={cls} className="dash-class-row">
                <span className="dash-class-name">{cls}</span>
                <div className="dash-class-track"><div className="dash-class-fill" style={{ width: `${Math.min(pct, 100)}%` }} /></div>
...
const Portfolio = () => {
  const {
    ...
    totalCryptoUSD, binanceSnap, loading, bobRate,
  } = useApp();
  ...
  // allAssets: todos los activos para buildPortfolioV3 (incluyendo patrimonio)
  ...
  // totalUSD bruto para el análisis (sin filtro de visibilidad)
  ...
  // filteredAssets: solo activos del portafolio (sin patrimonio) para la lista
  ...
  const handleCopy   = async () => {
    try { await navigator.clipboard.writeText(JSON.stringify(v3, null, 2)); setCopied(true); setTimeout(() => setCopied(false), 1800); } catch {}
  };

  const visibleManualUSD = visiblePieData.filter(i => i.type === 'manual' || i.type === 'stock').reduce((s, i) => s + i.valueUSD, 0);

  if (loading) return <div className="portfolio-loading"><div className="portfolio-spinner" /></div>;

  return (
    <div className="portfolio-page">

      {/* Hero — distribución visual */}
      <section className="portfolio-card portfolio-hero-card">
        ...
        <div className="portfolio-group-toggles">
          {groupDefinitions.map(group => {
            ...
            return (
              <button key={group.groupKey} type="button" onClick={() => toggleGroup(group.groupKey)}
        ...
        <div className="portfolio-overview-grid">
          ...
          <div className="portfolio-summary-panel">
            ...
            <button type="button" onClick={() => setLegendOpen(p => !p)} className="portfolio-legend-toggle" aria-expanded={legendOpen}>
              <span>Desglose</span>
      ...
      {/* Panel de Patrimonio (separado, no afecta métricas) */}
      {v3?.patrimony?.totalUSD > 0 && (
        <section className="portfolio-card">
          <div className="portfolio-section-head compact">
            <div>
              <p className="portfolio-eyebrow patrimony-eyebrow">Bienes no líquidos</p>
              <h2 className="portfolio-subtitle">Patrimonio</h2>
            </div>
            <Home size={16} className="text-orange-400/50" />
          </div>
      ...
      <section className="portfolio-tabs-wrap">
        <div className="portfolio-tabs-scroll">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`portfolio-tab ${activeTab === tab ? 'active' : ''}`}>{tab}</button>
          ))}
        </div>
      </section>

      {/* Lista de activos (solo portafolio, sin patrimonio) */}
          ...
          return (
            <article key={a.id} className="portfolio-asset-card">
              <div className="portfolio-asset-main">
                <div className="portfolio-asset-icon"><TypeIcon type={a.type} symbol={a.symbol} /></div>
