...
export default function CategoryBreakdown({
            ...
            {filteredByCategory.map(({ key, total, label, emoji }) => (
              <div key={key} className="space-y-1">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="text-white/50">Bs {total.toLocaleString('es-BO', { maximumFractionDigits: 0 })}</span>
                  <span className="text-white/30 ml-1">
                    {((total / byGroup.find((g) => g.key === activeGroup)?.total) * 100).toFixed(0)}%
                  </span>
                </div>
                <Bar pct={(total / filteredByCategory[0]?.total) * 100} color={GROUP_COLORS[activeGroup] || 'bg-white/20'} />
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div className="bg-brand-card rounded-2xl border border-white/5 p-4 space-y-3 analytics-card">
      ...
      {byCategory.length === 0 ? (
        <p className="text-white/30 text-sm text-center py-4">Sin gastos registrados</p>
      ) : (
