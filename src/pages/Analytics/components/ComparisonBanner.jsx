...
export default function ComparisonBanner({ period, prevRange, prevTotalExp, prevTotalInc, totalExp, totalInc }) {
  ...
  return (
    <div ref={ref} style={{ opacity: 0 }} className="bg-[#1f1f1f] rounded-lg border border-white/5 p-3">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1.5">
          <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#a855f7', display: 'inline-block' }} />
          <span className="text-10px font-bold text-[#eeeeee]/40 uppercase tracking-wide">vs período anterior</span>
        </div>
        <span className="text-10px text-[#eeeeee]/25">{fmtDate(prevRange.start)} → {fmtDate(prevRange.end)}</span>
      </div>

      <div className="grid grid-cols-3 gap-2">
        {rows.map(({ label, value, prev, color, invert }) => (
          <div key={label} className="bg-[#1f1f1f]/3 rounded-xl p-2">
