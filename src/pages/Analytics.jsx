...
const Sparkline = ({ data, color = '#2b7fff' }) => {
...
function getInsights(savingsRate, byGroup, totalExp, monthlyTrend, expenses) {
  ...
  if (savingsRate < 0) {
    tips.push({
      icon: '⚠️',
      color: 'text-rose-400',
      msg: 'Tus gastos superan tus ingresos en este perodo. Revisa tus compras discrecionales.',
    });
  } else if (savingsRate < 15) {
    tips.push({
      icon: '💡',
      color: 'text-yellow-400',
      msg: `Tu tasa de ahorro est en ${savingsRate.toFixed(0)}%. Intenta acercarte a la meta del 20%.`,
    });
  } else if (savingsRate >= 20) {
    tips.push({
      icon: '🎯',
      color: 'text-emerald-400',
      msg: `Excelente disciplina! Ests ahorrando e invirtiendo el ${savingsRate.toFixed(0)}% de tus ingresos.`,
    });
  }
  ...
  if (topGroup && totalExp > 0) {
    ...
    const groupLabel =
      TX_GROUPS?.find((g) => g.value === topGroup.key)?.label || topGroup.key;
    if (pct > 35) {
      tips.push({
        icon: '🔴',
        color: 'text-orange-400',
        msg: `${groupLabel} absorbe el ${pct.toFixed(0)}% de tus egresos. Alerta especfica de compras.`,
      });
    }
  }
  ...
  const salidasYOcio = expenses
    .filter(
      (e) =>
        e.category === 'citassalidas' || e.category === 'comidafuera'
    )
  ...
  if (totalExp > 0 && salidasYOcio / totalExp > 0.25) {
    tips.push({
      icon: '🍽️',
      color: 'text-pink-400',
      msg: 'Las salidas y restaurantes representan ms del 25% de tus gastos actuales.',
    });
  }
      ...
      >
        <span
          style={{
            ...
            flexShrink: 0,
            boxShadow: `0 0 6px ${slice.hex}88`,
          }}
        />
        <span
          style={{
...
const DonutAnalytics = ({
  ...
  const slices = useMemo(() => {
    ...
    return byGroup.map(({key, total}) => {
      ...
      const count = expenses.filter(
        (e) => e.parentCategory === key || e.group === key
      ).length;
      return {
        key,
  ...
  useEffect(() => {
    ...
    animate(
      svgRef.current.querySelectorAll('.dona-slice'),
      { opacity: [0, 1] },
  ...
  const handleEnter = useCallback((slice, e) => {
    ...
    if (sliceRefs.current[slice.key]) {
      animate(
        sliceRefs.current[slice.key],
        { scale: 1.04 },
        { duration: 150, ease: 'outSine' }
      );
    }
  }, []);
  ...
  const handleClick = useCallback(
    (slice) => {
      if (sliceRefs.current[slice.key]) {
        animate(
          sliceRefs.current[slice.key],
          { scale: [1.04, 0.96, 1.02, 1] },
          { duration: 350, ease: 'outElastic(1, 0.5)' }
        );
      }
  ...
  const activeTotal =
    byGroup.find((g) => g.key === activeGroup)?.total || 0;
  ...
  return (
    ...
    >
      <div style={{ position: 'relative', width: '100%', maxWidth: 280 }}>
        ...
        >
          ...
          >
            Bs{' '}
            {activeGroup
              ? activeTotal.toLocaleString('es-BO', {
                  maximumFractionDigits: 0,
                })
              : totalExp.toLocaleString('es-BO', {
                  maximumFractionDigits: 0,
                })}
          </text>
          {activeGroup && (
            <text
              ...
              fontFamily="JetBrains Mono,monospace"
            >
              {slices.find((s) => s.key === activeGroup)?.pct.toFixed(1)}% del
              total
            </text>
          )}
        </svg>
        ...
        {slices.map((s) => {
          ...
          return (
            <button
              key={s.key}
              type="button"
              onClick={() => onGroupClick(isActive ? null : s.key)}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: '0.4rem',
            ...
            >
              <span
                style={{
                  ...
                  background: s.hex,
                  flexShrink: 0,
                  boxShadow: isActive ? `0 0 6px ${s.hex}` : 'none',
                }}
              />
              <span
                style={{
                  fontSize: '0.62rem',
                  fontWeight: 800,
                  color: isActive ? s.hex : 'rgba(255,255,255,0.5)',
                  textTransform: 'uppercase',
              ...
              <span
                style={{
                  fontSize: '0.58rem',
                  color: isActive ? s.hex : 'rgba(255,255,255,0.25)',
                  fontFamily: 'JetBrains Mono,monospace',
                }}
              >
                {s.pct.toFixed(0)}%
      ...
      {hoveredSlice && anchorPos && (
        <DonutTooltip slice={hoveredSlice} anchorPos={anchorPos} />
      )}
    </div>
  );
};
...
const DateRangePicker = ({
  ...
  return (
    <div
      ref={pickerRef}
      className="bg-[#1f1f1f] border border-[#262626] rounded-lg p-4 space-y-3"
      style={{ opacity: 0 }}
    >
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          <Calendar size={13} className="text-[#2b7fff]" />
          <span className="text-xs font-bold text-[#eeeeee]/70">
            Rango personalizado
          </span>
        </div>
        <button
          type="button"
          onClick={onClear}
          className="text-[#eeeeee]/30 hover:text-[#eeeeee]/60 transition-colors"
        >
          <X size={14} />
        </button>
      </div>
        ...
        {[
          {
            ...
            max: toInputDate(endDate || new Date()),
            min: undefined,
            onChange: (e) =>
              onStartChange(
          ...
          <div key={label}>
            <label className="text-10px text-[#eeeeee]/40 block mb-1.5 font-semibold uppercase tracking-wide">
              {label}
            </label>
            <input
              type="date"
            ...
            />
            {display && (
              <p className="text-10px text-[#2b7fff] mt-1 ml-1">
                {fmtDate(display)}
              </p>
            )}
          </div>
        ...
        {[
          {
            ...
            fn: () => {
              const n = new Date();
              onStartChange(new Date(n.getFullYear(), n.getMonth(), 1));
          ...
          },
          {
            ...
            fn: () => {
              const n = new Date();
              onStartChange(new Date(n.getFullYear(), n.getMonth() - 1, 1));
              onEndChange(new Date(n.getFullYear(), n.getMonth(), 0));
            },
          },
          {
            label: 'Ao 2026',
            fn: () => {
              onStartChange(new Date('2026-01-01'));
              onEndChange(new Date('2026-12-31'));
            },
          },
        ].map(({ label, fn }) => (
          <button
      ...
      {startDate && endDate && (
        <div className="bg-[#2b7fff]/10 border border-[#2b7fff]/20 rounded-xl px-3 py-2">
          <p className="text-11px text-[#2b7fff] font-semibold">
            {fmtDate(startDate)} → {fmtDate(endDate)}
          </p>
          <p className="text-10px text-[#eeeeee]/40 mt-0.5">
            {Math.round((endDate - startDate) / 86400000) + 1} das
            seleccionados
          </p>
        </div>
      )}
...
const ComparisonBanner = ({
  ...
  return (
    <div
      ref={ref}
      style={{ opacity: 0 }}
      className="bg-[#1f1f1f] rounded-lg border border-white/5 p-3"
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1.5">
          ...
          />
          <span className="text-10px font-bold text-[#eeeeee]/40 uppercase tracking-wide">
            vs periodo anterior
          </span>
        </div>
        <span className="text-10px text-[#eeeeee]/25">
          {fmtDate(prevRange.start)} → {fmtDate(prevRange.end)}
        </span>
      </div>
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          },
          {
            ...
            prev: Math.abs(prevBalance),
            color: balance >= 0 ? 'text-teal-400' : 'text-rose-400',
            invert: false,
          },
        ].map(({ label, value, prev, color, invert }) => (
          <div
            key={label}
            className="bg-[#1f1f1f]/3 rounded-xl p-2"
          >
            <p className="text-9px text-[#eeeeee]/30 mb-1">{label}</p>
            ...
            <p className="text-9px text-[#eeeeee]/20 mt-0.5">
              ant: Bs{' '}
              {prev.toLocaleString('es-BO', { maximumFractionDigits: 0 })}
            </p>
          </div>
        ))}
      </div>
...
const CategoryRow = ({
  ...
  const expensesTotal = Number(totalExp) || 0;
  const groupMax = Number(maxGroup) || 1;

  const color = GROUPCOLORS[groupKey] || 'bg-[#1f1f1f]/20';
  const text = GROUPTEXT[groupKey] || 'text-[#eeeeee]/60';
  const hex = GROUPHEX[groupKey] || 'rgba(255,255,255,0.3)';

  const groupLabel =
    TX_GROUPS?.find((group) => group.value === groupKey)?.label ||
    groupKey;

  // Cada elemento conserva su total de subcategoría.
  const subItems = categories
    .filter((category) => category.parent === groupKey)
    .map((category) => ({
      key: category.key,
      subTotal: Number(category.total) || 0,
  ...
  const subMax = Math.max(
    ...subItems.map((item) => item.subTotal),
    1
  );
  ...
  useEffect(() => {
    ...
    animate(
      subRef.current.querySelectorAll('.sub-cat-row'),
      {
        opacity: [0, 1],
        translateX: [-8, 0],
      },
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        <div className="flex justify-between items-center text-xs">
          <div className="flex items-center gap-1.5">
            ...
            {subItems.length > 0 && (
              <span className="text-10px text-[#eeeeee]/20">
                {subItems.length} cat.
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-[#eeeeee]/80">
              Bs{' '}
              {groupTotal.toLocaleString('es-BO', {
                maximumFractionDigits: 0,
              })}
            </span>
          </div>
      ...
      {open && subItems.length > 0 && (
        <div
          ref={subRef}
          className="pl-4 space-y-2 mt-1 border-l border-white/5 ml-1.5"
        >
          {subItems.map((item) => {
            const {
              key,
            ...
            const subPercentage =
              groupTotal > 0
                ? (subTotal / groupTotal) * 100
                : 0;

            const subBarPercentage =
              subMax > 0
                ? (subTotal / subMax) * 100
                : 0;
            ...
            return (
              <div
                key={key}
                className="sub-cat-row space-y-1"
                style={{ opacity: 0 }}
              >
                <div className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[#eeeeee]/25 text-10px">
                ...
                <div className="h-1 bg-[#1f1f1f]/5 rounded overflow-hidden">
                  <div
                    className="h-full rounded opacity-60 transition-all duration-500"
                    style={{
                      width: `${Math.min(
                        subBarPercentage,
                        100
...
const authLoading = auth?.loading ?? true;
  ...
  // Rango memoizado para evitar ciclo infinito
  const formatQueryDate = useCallback((value) => {
...
const {
  ...
  const totalExp = useMemo(
    () => expenses.reduce((s, tx) => s + tx.amount, 0),
    [expenses]
  );
  const totalInc = useMemo(
    () => incomes.reduce((s, tx) => s + tx.amount, 0),
    [incomes]
  );
  const balance = totalInc - totalExp;
  ...
  const prevExpenses = useMemo(
    () => prevFiltered.filter((tx) => tx.type === 'expense'),
    [prevFiltered]
  );
  const prevIncomes = useMemo(
    () => prevFiltered.filter((tx) => tx.type === 'income'),
    [prevFiltered]
  );

  const prevTotalExp = useMemo(
    () => prevExpenses.reduce((s, tx) => s + tx.amount, 0),
    [prevExpenses]
  );
  const prevTotalInc = useMemo(
    () => prevIncomes.reduce((s, tx) => s + tx.amount, 0),
    [prevIncomes]
  );
  ...
  // Auxiliares de Clculo
  const maxGroup = byGroup[0]?.total || 1;
  const filteredByCategory = activeGroup
    ? byCategory.filter((c) => c.parent === activeGroup)
    : byCategory;
  ...
  const periodLabel = useMemo(() => {
    if (period === 'custom') {
      ...
      if (customStart) {
        return `Desde ${fmtDate(customStart)}`;
      }
  ...
  return (
    <div className="space-y-5 pb-24">
      ...
      <div className="flex justify-between items-center pt-2">
        <div>
          <h1 className="text-2xl font-bold">Anlisis Financiero</h1>
          {(period === 'custom' || customStart || customEnd) && (
            <p className="text-11px text-[#2b7fff] mt-0.5">{periodLabel}</p>
          )}
        </div>
        <div className="flex gap-1">
          <button
            ...
            className={`p-2 rounded-xl text-sm transition-colors ${
              viewMode === 'groups'
                ? 'bg-[#2b7fff] text-[#eeeeee]'
                : 'bg-[#1f1f1f] text-[#eeeeee]/40'
            }`}
          >
            <PieChart size={16} />
          </button>
      ...
      <ComparisonBanner
        period={period}
        prevRange={prevRange}
        prevTotalExp={prevTotalExp}
        prevTotalInc={prevTotalInc}
        totalExp={totalExp}
        totalInc={totalInc}
      />
      ...
      <div ref={cardsRef} className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3 analytics-card">
          {[
            ...
            },
            {
              label: 'Balance',
              value: Math.abs(balance),
              prev: Math.abs(prevTotalInc - prevTotalExp),
              color: balance >= 0 ? 'text-teal-400' : 'text-rose-400',
              invert: false,
            },
          ].map(({ label, value, prev, color, invert }) => (
            <div
              key={label}
              className="bg-[#1f1f1f] rounded-lg p-3 border border-white/5"
            >
        ...
        <div className="grid grid-cols-2 gap-3 analytics-card">
          ...
          <div className="bg-[#1f1f1f] rounded-lg p-4 border border-white/5">
            <div className="flex justify-between items-start mb-2">
              ...
              <Sparkline
                data={monthlyTrend.map((m) => m.exp)}
                color="#2b7fff"
              />
            </div>
            <p className="text-xl font-bold text-[#eeeeee]/80">
              Bs{' '}
              {avgMonthlyExp.toLocaleString('es-BO', {
                maximumFractionDigits: 0,
              })}
            </p>
          ...
          {projectedExp !== null && (
            <div className="bg-[#1f1f1f] rounded-lg p-4 border border-white/5">
              ...
              <p
                className={`text-xl font-bold ${
                  projectedExp > currentMonthInc
                    ? 'text-rose-400'
                    : 'text-[#eeeeee]/80'
                }`}
              >
                Bs{' '}
                {projectedExp.toLocaleString('es-BO', {
                  maximumFractionDigits: 0,
                })}
              </p>
              <p className="text-10px text-[#eeeeee]/30 mt-1">
                Ritmo actual (da {now.getDate()})
              </p>
            </div>
          )}
          ...
          {period === '1m' && (
            <div className="bg-[#1f1f1f] rounded-lg p-4 border border-white/5">
              <div className="flex justify-between items-start mb-2">
                ...
                <span className="text-10px text-[#eeeeee]/30">
                  {daysLeft} dias restantes
                </span>
              </div>
              <p
                className={`text-xl font-bold ${
                  dailyBudget > 0 ? 'text-teal-400' : 'text-rose-400'
                }`}
              >
                Bs{' '}
                {Math.max(dailyBudget, 0).toLocaleString('es-BO', {
                  maximumFractionDigits: 0,
                })}
              </p>
              <p className="text-10px text-[#eeeeee]/30 mt-1">
          ...
          <div className="flex items-end gap-2 h-20">
            {monthlyTrend.map((m, i) => {
              const maxVal = Math.max(
                ...monthlyTrend.map((x) => Math.max(x.exp, x.inc)),
                1
              );
                  ...
                  <div className="w-full flex gap-0.5 items-end h-16">
                    <div
                      className={`flex-1 rounded-t transition-all duration-500 ${
                        isLast ? 'bg-rose-500' : 'bg-rose-500/40'
                      }`}
                      style={{
                        height: `${(m.exp / maxVal) * 100}%`,
                      }}
                    />
                    <div
                      className={`flex-1 rounded-t transition-all duration-500 ${
                      ...
                      style={{
                        height: `${(m.inc / maxVal) * 100}%`,
                      }}
                    />
                  </div>
                  <span className="text-9px text-[#eeeeee]/30 capitalize">
                    {m.label}
                  </span>
                </div>
              );
        ...
        <div className="bg-[#1f1f1f] rounded-lg border border-white/5 p-4 space-y-3 analytics-card">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm">Diagnstico financiero</h3>
          </div>
          {insights.map((tip, i) => (
        ...
        {viewMode === 'groups' && (
          <div className="bg-[#1f1f1f] rounded-lg border border-white/5 p-4 space-y-4 analytics-card">
            ...
            {byGroup.length === 0 ? (
              <p className="text-[#eeeeee]/30 text-sm text-center py-4">
                Sin datos registrados en este perodo
              </p>
            ) : (
              <DonutAnalytics
                byGroup={byGroup}
              ...
              <div className="space-y-3 pt-2 border-t border-white/5">
                {byGroup.map(({ key, total }) => (
                  <CategoryRow
                    ...
                    maxGroup={maxGroup}
                    prevTotal={prevByGroup[key] ?? null}
                  />
                ))}
              </div>
            )}
                  ...
                  <div key={key} className="space-y-1">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="text-[#eeeeee]/30 ml-1">
                        {(
                          (total / byGroup.find((g) => g.key === activeGroup)?.total) *
                          100
                        ).toFixed(0)}
                        %
                      </span>
                    ...
                    <Bar
                      pct={(total / filteredByCategory[0]?.total) * 100}
                      color={GROUP_COLORS[activeGroup] || 'bg-[#1f1f1f]/20'}
                    />
                  </div>
                ))}
              </div>
        ...
        {/* Vista por Categora Plana */}
        {viewMode === 'categories' && (
          <div className="bg-[#1f1f1f] rounded-lg border border-white/5 p-4 space-y-3 analytics-card">
            <h3 className="font-bold text-sm">Detalle por categora</h3>
            {byCategory.length === 0 ? (
              <p className="text-[#eeeeee]/30 text-sm text-center py-4">
              ...
              <div className="space-y-3">
                {byCategory.map(({ key, total, label, emoji, parent }) => (
                  <div key={key} className="space-y-1">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-semibold flex items-center gap-1.5">
                        <span
                          ...
                          style={{
                            background:
                              GROUP_HEX[parent] || 'rgba(255,255,255,0.2)',
                          }}
                        />
                        {emoji} {label}
                      </span>
                    ...
                    <Bar
                      pct={(total / byCategory[0]?.total) * 100}
                      color={GROUP_COLORS[parent] || 'bg-[#1f1f1f]/20'}
                    />
                  </div>
                ))}
              </div>
        ...
        <div className="bg-[#1f1f1f] rounded-lg border border-white/5 p-4 space-y-2 analytics-card">
          <h3 className="font-bold text-sm mb-3">Mayores egresos del perodo</h3>
          {expenses.length === 0 ? (
            <p className="text-[#eeeeee]/30 text-sm text-center py-3">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="font-semibold text-[#eeeeee]/90">
                            {tx.concept || tx.title || 'Gasto'}
                          </p>
                      ...
                      <span className="text-rose-400 font-bold">
                        Bs{' '}
                        {tx.amount.toLocaleString('es-BO', {
                          maximumFractionDigits: 0,
                        })}
                      </span>
                    </div>
