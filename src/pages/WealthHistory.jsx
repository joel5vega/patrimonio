...
const SPECIAL_TODO = {
  key: 'todo_full', label: 'Todo', field: 'todo_full', color: '#22c55e',
};

// ─── Roles derivados dinámicamente de cada snapshot ──────────────────────────
// NO son campos guardados en Firestore — se calculan en el frontend
const ROLE_TYPES = [
...
// Lista negra: campos manual_* que NO son trading de Quantfury
const EXCLUDED_FROM_TRADING = new Set(['AirTM', 'Ahorro', 'AhorroBs', 'SAFI', 't ACH', 't SAS']);

function classifyManualField(logicalName) {
  const lower = logicalName.toLowerCase().trim();
  if (lower === 'airtm')                                         return 'yield';
  if (['ahorro', 'ahorrobs', 'safi'].includes(lower))           return 'reserve';
  if (['t ach', 't sas'].includes(lower))                       return 'patrimony';
  return 'trading'; // cualquier otro manual_* → Quantfury acumulado
}

// Suma USD por rol a partir de un snapshot row completo
function computeRolesFromRow(row, bobRate) {
  ...
  Object.entries(row).forEach(([key, val]) => {
    ...
    const logicalName = key.replace('manual_', '').trim(); // ← trim()
    const usd = logicalName.toLowerCase() === 'ahorrobs'
      ? (typeof val === 'number' ? val / (bobRate || 6.96) : 0)
      : (typeof val === 'number' ? val : 0);
    const role = classifyManualField(logicalName);
...
const MultiLineChart = ({ series, onHover, period }) => {
  ...
  const toX = (i, len) => PAD_L + (i / Math.max(len - 1, 1)) * (W - PAD_L - PAD_R);
  const toY = (v)       => PAD_T + (1 - (v - min) / range) * (H - PAD_T - PAD_B);
  ...
  const handleMove = (evt) => {
    ...
    if (onHover) onHover({ index: idx, date: primary.data[idx].date });
  };
  ...
  const pickLabeledPoints = () => {
    ...
    if (period === '1M') {
      ...
      primary.data.forEach((d, i) => {
        ...
        if (!lastDate || (curr - lastDate) / 86400000 >= 7) { result.push({ ...d, index: i }); lastDate = curr; }
      });
      return result;
    }
    if (period === '3M' || period === '1Y') {
      const map = new Map();
      primary.data.forEach((d, i) => { const k = d.date.slice(0, 7); if (!map.has(k)) map.set(k, { ...d, index: i }); });
      return Array.from(map.values());
    }
    if (period === 'ALL') {
      const map = new Map();
      primary.data.forEach((d, i) => { const k = d.date.slice(0, 4); if (!map.has(k)) map.set(k, { ...d, index: i }); });
      return Array.from(map.values());
    }
    return [];
  };
  ...
  const xAxisLabels = (() => {
    if (!primary.data.length) return [];
    if (period === '1W')  return primary.data.map((d, i) => ({ index: i, date: d.date.slice(5) }));
    if (period === '1M')  return pointLabels.map((p) => ({ index: p.index, date: p.date.slice(5) }));
    if (period === '3M' || period === '1Y') return pointLabels.map((p) => ({ index: p.index, date: p.date.slice(0, 7) }));
    if (period === 'ALL') return pointLabels.map((p) => ({ index: p.index, date: p.date.slice(0, 4) }));
    return [];
  })();
  ...
  return (
    ...
    >
        ...
        return (
          <g key={i}>
            ...
            <text x={PAD_L - 4} y={y + 3.5} textAnchor="end" fontSize="8"
              fill="rgba(255,255,255,0.35)" fontFamily="monospace">{fmt(val)}</text>
          </g>
        );
      })}
      ...
      {xAxisLabels.map((l) => {
        ...
        return (
          <g key={l.date}>
            <line x1={x} y1={H - PAD_B} x2={x} y2={H - PAD_B + 3}
              stroke="rgba(255,255,255,0.2)" strokeWidth="1" />
            <text x={x} y={H - 4} textAnchor="middle" fontSize="8"
              fill="rgba(255,255,255,0.4)" fontFamily="system-ui, -apple-system, BlinkMacSystemFont">
              {l.date}
            </text>
          </g>
        );
      ...
      {pointLabels.map((p) => {
        ...
        return (
          <g key={p.date}>
            ...
            <rect x={x - 16} y={y - 14} width={32} height={10} rx={3} fill="rgba(15,23,42,0.9)" />
            <text x={x} y={y - 7} textAnchor="middle" fontSize="7" fill="#e5e7eb"
              fontFamily="system-ui, -apple-system, BlinkMacSystemFont">{fmt(p.v)}</text>
          </g>
        );
      })}
    </svg>
...
// ─── Fila reutilizable de desglose ─────────────────────────────────────────────
const BreakdownRow = ({ row, vis, toggleVisible, totalUSD }) => (
  <div
...
const WealthHistory = () => {
    ...
    () =>
      (manualAssets ?? []).map((a, i) => {
        ...
        const valueUSD   = isAhorroBs
          ...
          : a.valueUSD;
        const isTrading = !EXCLUDED_FROM_TRADING.has(fieldName);
        return {
  ...
  console.log('Snapshot más reciente:', latest);
  const roles = computeRolesFromRow(latest, bobRate);
  console.log('Roles calculados:', roles);
    return computeRolesFromRow(latest, bobRate);
  ...
  const seriesData = useMemo(() => {
    ...
    const inPeriod = (dateStr) => days >= 9999 || new Date(dateStr) >= cutoff;

    const allSorted = chartHistory?.length
      ? [...chartHistory].sort((a, b) => a.date.localeCompare(b.date))
      : [];

    const result = {};

    for (const asset of FIXED_TYPES) {
      result[asset.key] = allSorted
        .map((d) => d[asset.field] != null ? { date: d.date, v: d[asset.field] } : null)
        .filter(Boolean)
    ...
    if (totalSeries.length) {
      const mapBs = new Map(
        allSorted.map((d) => [
          d.date,
          d['manual_AhorroBs'] != null ? d['manual_AhorroBs'] / (bobRate || 6.96) : 0,
        ])
      );
      result['todo_full'] = totalSeries
    ...
    for (const roleType of ROLE_TYPES) {
      const roleName = roleType.key.replace('role_', '');
      result[roleType.key] = allSorted
        .map((d) => {
          const roles = computeRolesFromRow(d, bobRate || 6.96);
          const v = roles[roleName] ?? 0;
    ...
    for (const asset of manualTypes) {
      result[asset.key] = allSorted
        .map((d) => {
          const raw = d[asset.field];
          if (raw == null) return null;
          const v = asset.isAhorroBs && bobRate ? raw / bobRate : raw;
    ...
    return result;
  }, [chartHistory, days, manualTypes, bobRate]);
  ...
  const tradingManualTypes    = manualTypes.filter((t) => t.isTrading && (t.valueUSD ?? 0) > 0);
  const nonTradingManualTypes = manualTypes.filter((t) => !t.isTrading && (t.valueUSD ?? 0) > 0);
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {[SPECIAL_TODO, ...FIXED_TYPES].map((t) => (
            <ChipButton key={t.key} type={t} />
          ))}
        </div>

        <p className="text-[10px] text-white/30 uppercase tracking-wider pt-1">
          Por categoría
        </p>
        <div className="flex flex-wrap gap-1.5">
        ...
        {tradingManualTypes.length > 0 && (
          <>
            <p className="text-[10px] text-white/30 uppercase tracking-wider pt-1">
              Quantfury — individual
            </p>
            <div className="flex flex-wrap gap-1.5">
              {tradingManualTypes.map((t) => <ChipButton key={t.key} type={t} />)}
            </div>
          </>
        )}

        {nonTradingManualTypes.length > 0 && (
          <>
            <p className="text-[10px] text-white/30 uppercase tracking-wider pt-1">
              Activos manuales
            </p>
            <div className="flex flex-wrap gap-1.5">
              {nonTradingManualTypes.map((t) => <ChipButton key={t.key} type={t} />)}
            </div>
          </>
        )}
      </div>
        ...
        {[
          ...
          .map((row) => (
            <BreakdownRow
              ...
              totalUSD={currentTotalUSD + ahorroBsUSD}
            />
          ))}

        <p className="text-[10px] text-white/30 uppercase tracking-wider pt-2">
          Por categoría
        </p>
        {[
